...
final class Plugin {
	const HOME_PAGE_OPTION = 'unikon_webmcp_demo_home_page_id';
	...
	const HOME_SHORTCODE = 'unikon_webmcp_home';
	...
	public function run() {
		...
		add_shortcode( self::HOME_SHORTCODE, array( $this, 'render_home' ) );
	}

	public static function maybe_ensure_pages() {
		if ( current_user_can( 'manage_options' ) && ( ! get_option( self::HOME_PAGE_OPTION ) || ! get_option( self::DESIGN_PAGE_OPTION ) || ! get_option( self::SEWING_PAGE_OPTION ) ) ) self::activate();
	}

	public static function activate() {
		self::ensure_page( self::HOME_PAGE_OPTION, 'fashion-learning-studio-home', __( 'Fashion Learning Studio', 'unikon-webmcp-demo' ), '[' . self::HOME_SHORTCODE . ']' );
	...
	/** Render the public landing page; learning tools remain on authenticated course pages. */
	public function render_home() {
		Assets::enqueue();
		$course_page_id = (int) get_option( self::PAGE_OPTION );
		$course_url = $course_page_id ? get_permalink( $course_page_id ) : home_url( '/fashion-learning-studio/' );
		$demo_url = is_user_logged_in() ? $course_url : wp_login_url( $course_url );
		$course_links = array();
		...
		ob_start();
		include UNIKON_WEBMCP_DEMO_DIR . 'public/partials/home.php';
